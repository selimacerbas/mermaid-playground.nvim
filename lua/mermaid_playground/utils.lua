...
function M.base64_urlencode(data)
    ...
    for _ = 1, pad do table.insert(bytes, 0) end
    local out = {}
    for i = 1, #bytes, 3 do
...
function M.open_in_browser(url, cmd)
    if cmd and #cmd > 0 then
        vim.fn.jobstart({ cmd, url }, { detach = true }); return
    end
...
function M.detect_icon_packs(src)
    ...
    for p in src:gmatch("%f[%w]([%l%d%-]+):[%l%d%-]+%f[^%w]") do
        if not seen[p] then
            seen[p] = true; table.insert(names, p)
        end
    end
    return names
end
...
-- Helpers
local function ts_text(node, bufnr) return vim.treesitter.get_node_text(node, bufnr) end
local function named_child_by_type(node, t)
    if not node then return nil end
    local n = node:named_child_count()
    for i = 0, n - 1 do
        local c = node:named_child(i)
        if c and c:type() == t then return c end
    end
    return nil
end
...
local function get_queries(lang)
    ...
    local ok1, q1 = pcall(ts_query.parse, lang, [[
    (fenced_code_block (info_string) @info (code_fence_content) @code) @block
  ]])
    -- 2) Fallback: capture blocks only, we’ll search children in Lua
    local ok2, q2 = pcall(ts_query.parse, lang, [[
...
function M.treesitter_mermaid_under_cursor()
    ...
    local q_exact, q_blocks = get_queries("markdown")
    ...
    local function consider_block(block, info_node, code_node)
        ...
        if not info_node then info_node = named_child_by_type(block, "info_string") end
        if not code_node then code_node = named_child_by_type(block, "code_fence_content") end
        if not code_node then return nil end

        local sr, _, er, _ = block:range()
        if not (row >= sr and row <= er) then return nil end -- allow fence lines too
    ...
    if q_exact then
        for _, match, _ in q_exact:iter_matches(root, bufnr, 0, -1) do
            ...
            if src and #src > 0 then return true, src end
        end
    end

    -- Fallback: block-only query and manual child discovery
    if q_blocks then
...
function M.regex_mermaid_under_cursor()
    ...
    local function fence_info(line)
        ...
        if not fence then return nil, false end
        rest = rest or ""
    ...
    for i = cur, 1, -1 do
        ...
        if f then
            if ok then
                open_row, fence = i, f; break
            else return false end
        end
    end
    if not open_row then return false end
    ...
    if not close_row then return false end

    -- allow cursor on fence lines or inside
    if cur < open_row or cur > close_row then return false end
